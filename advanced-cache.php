...
if (is_file($cache_file) && is_readable($cache_file) && (filemtime($cache_file) + $ttl) > time()) {
    ...
    if (is_string($ucp_cached_html) && '' !== $ucp_cached_html) {
        ...
        if (($if_none_match && $if_none_match === $etag) || ($if_modified_since && strtotime($if_modified_since) >= $file_mtime)) {
        ...
        // Transparante encoding-compressie als de browser dit ondersteunt (Brotli > gzip).
