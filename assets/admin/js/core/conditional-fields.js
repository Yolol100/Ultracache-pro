document.addEventListener('DOMContentLoaded', function () {
  ...
  function getPrimaryControl(wrapper) {
    ...
    return wrapper.querySelector('[data-ucp-primary-control="1"]');
  }
  ...
  function evaluateConditionalFields(form) {
    ...
    form.querySelectorAll('[data-ucp-field-key]').forEach(function (wrapper) {
      ...
      if (parentKey) {
        const parentValue = getWrapperValue(form, parentKey);
        if (parentValue !== '1') {
          shouldDisable = true;
  ...
  window.ucpAdminCore.evaluateConditionalFields = evaluateConditionalFields;
