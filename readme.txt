...
= 4.22.6.1 =
* Maintenance release: accessibility, i18n and stricter SSL verification fixes.
* Packaged under the stable plugin folder `ultracache-pro` so WordPress upload can replace the existing plugin.
...
= 4.22.4 =
* Add: PageSpeed Auto preset for Elementor/WooCommerce sites with cache, Used CSS, Delay JS, local fonts, lazyload and safe checkout/account bypasses.
...
* Hardened Core Web Vitals collection with same-origin nonce verification and retained rate limiting.
...
* Limited and sanitized Early Hints preload headers, including safer font preload handling.
...
* Changed legacy duplicate plugin cleanup from automatic deletion to manual-review candidate recording.
...
* Added duplicate legacy UltraCache Pro plugin cleanup during activation when permissions allow.
...
* JavaScript minify can stay enabled with Defer JS and Delay JS; only JS combine/native script-strategy conflicts are auto-disabled.
= 4.21.28 =
