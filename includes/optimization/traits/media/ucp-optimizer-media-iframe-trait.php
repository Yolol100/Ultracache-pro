...
trait UCP_Optimizer_Media_Iframe_Trait {
    private function optimize_iframe_loading_attribute($matches) {
        ...
        if (UCP_Options::get('enable_lazy_youtube_preview') && preg_match('/\bsrc\s*=\s*["\']([^"\']+)["\']/i', $attrs, $src_match)) {
            ...
            if ($video_id) {
                ...
                if (!preg_match('/\btitle\s*=/i', $attrs)) {
                    $attrs .= ' title="' . esc_attr__('YouTube-video', 'ultracache-pro') . '"';
                }
    ...
    private function extract_youtube_video_id($src) {
        ...
        $youtube_hosts = array('youtube.com', 'www.youtube.com', 'm.youtube.com', 'music.youtube.com', 'youtube-nocookie.com', 'www.youtube-nocookie.com');
        $is_short_host = in_array($host, array('youtu.be', 'www.youtu.be'), true);
        if (!$is_short_host && !in_array($host, $youtube_hosts, true)) {
            return '';
        }
