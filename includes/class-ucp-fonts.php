...
class UCP_Fonts {
    ...
    public function localize_google_fonts($html, $handle, $href, $media) {
        if (empty(UCP_Options::get('enable_local_google_fonts')) || empty($href)) { return $html; }
        $host = strtolower((string) wp_parse_url($href, PHP_URL_HOST)); if ('fonts.googleapis.com' !== $host || !wp_http_validate_url($href)) { return $html; }
        $local = $this->cached_css_url($href); return $local ? str_replace(esc_url($href), esc_url($local), $html) : $html;
    }
    protected function cached_css_url($href) {
        $uploads = wp_upload_dir(); if (empty($uploads['basedir']) || empty($uploads['baseurl'])) { return false; }
        $dir = trailingslashit($uploads['basedir']) . 'ultracache-pro/fonts'; if (!wp_mkdir_p($dir)) { return false; }
        $file = trailingslashit($dir) . md5($href) . '.css';
        if (!file_exists($file)) {
            $this->schedule_refresh($href);
            return false;
        } elseif (filemtime($file) < (time() - WEEK_IN_SECONDS)) {
            $this->schedule_refresh($href);
        }
    ...
    public function refresh_cached_css($href) {
        ...
        $host = strtolower((string) wp_parse_url($href, PHP_URL_HOST)); if ('fonts.googleapis.com' !== $host || !wp_http_validate_url($href)) { return false; }
        $dir = trailingslashit($uploads['basedir']) . 'ultracache-pro/fonts'; if (!wp_mkdir_p($dir)) { return false; }
        $file = trailingslashit($dir) . md5($href) . '.css';
        $response = wp_remote_get($href, array('timeout' => 8, 'redirection' => 0, 'reject_unsafe_urls' => true, 'user-agent' => 'Mozilla/5.0 UltraCache Pro'));
        if (is_wp_error($response) || 200 !== (int) wp_remote_retrieve_response_code($response)) { return false; }
        $type = wp_remote_retrieve_header($response, 'content-type'); if ($type && false === stripos((string) $type, 'text/css')) { return false; }
        $body = wp_remote_retrieve_body($response); if (!$body || strlen($body) > 250000 || false !== stripos($body, '<html') || false !== stripos($body, '<?php')) { return false; }
    ...
    protected function schedule_refresh($href) {
        ...
        $href = esc_url_raw((string) $href);
        if (!$href || wp_next_scheduled('ucp_refresh_google_font_cache', array($href))) { return; }
    ...
    protected function write_cached_file($path, $body, $base_dir) {
        ...
        return false !== file_put_contents($path, $body, LOCK_EX);
    }
