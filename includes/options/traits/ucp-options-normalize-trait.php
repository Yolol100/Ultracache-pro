...
trait UCP_Options_Normalize_Trait {
    ...
    public static function settings_for_export($settings = null) {
        ...
        foreach (array_keys($settings) as $key) {
            if (preg_match('/(api|secret|token|license|zone_id|email)/i', (string) $key)) {
                unset($settings[$key]);
            }
        }
    ...
    public static function normalize($settings, $current = array()) {
        ...
        $settings['preload_critical_images'] = min(2, max(0, absint(isset($settings['preload_critical_images']) ? $settings['preload_critical_images'] : 1)));
        if (!empty($settings['active_preset']) && 'pagespeed_auto' === $settings['active_preset']) {
            $settings['preload_critical_images'] = min(2, max(1, absint($settings['preload_critical_images'])));
        }
