...
trait UCP_Installer_Lifecycle_Trait {
    ...
    protected static function activate_single_site() {
        ...
        UCP_Helpers::ensure_cache_dirs();
        self::cleanup_previous_version_artifacts();
        self::record_legacy_plugin_duplicates();
    ...
    /**
     * Remove cache artifacts generated by previous UltraCache versions and refresh the
    ...
    /**
     * Record older duplicate UltraCache Pro installs for manual review.
     *
     * Activation must never delete or deactivate other plugin folders automatically;
     * administrators can use the stored candidate list to clean up legacy copies
     * after checking backups and the active plugin state.
     */
    protected static function record_legacy_plugin_duplicates() {
        if (!is_admin()) {
            return;
        }
        ...
        foreach ($plugins as $basename => $data) {
            ...
            $folder = dirname((string) $basename);
            $is_duplicate = ('UltraCache Pro' === $name || in_array($folder, array(
                'ultracache-pro-old',
        ...
        if (!empty($candidates)) {
            update_option('ucp_legacy_plugin_cleanup_candidates', array_values($candidates), false);
        } else {
            delete_option('ucp_legacy_plugin_cleanup_candidates');
        }
    }
}
