...
class UCP_Site_Health {
    ...
    public static function test_cache_dir() {
        $ok = file_exists(UCP_CACHE_DIR) && is_writable(UCP_CACHE_DIR);
        return array(
    ...
    public static function test_job_backlog() {
        ...
        return array(
            ...
            'badge'       => array('label' => __('Snelheid', 'ultracache-pro'), 'color' => 'blue'),
            'description' => '<p>' . esc_html(sprintf(__('Nu in de wachtrij: %d open, %d mislukt.', 'ultracache-pro'), $pending, $failed)) . '</p>',
