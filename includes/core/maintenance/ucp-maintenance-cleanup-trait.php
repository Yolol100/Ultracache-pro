...
trait UCP_Maintenance_Cleanup_Trait {
    ...
    protected static function quoted_table($table) {
    ...
    public static function cleanup_logs() {
        ...
        if (self::is_safe_table_name(UCP_TABLE_LOGS)) {
            $table = self::quoted_table(UCP_TABLE_LOGS);
    ...
    public static function cleanup_diagnostics() {
        ...
        $cutoff = gmdate('Y-m-d H:i:s', time() - ($days * DAY_IN_SECONDS));
        if (self::is_safe_table_name(UCP_TABLE_DIAGNOSTICS)) {
            $table = self::quoted_table(UCP_TABLE_DIAGNOSTICS);
            // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- plugin-owned table identifier is validated and quoted before use; value is prepared.
    ...
    public static function cleanup_jobs() {
        ...
        $cutoff = gmdate('Y-m-d H:i:s', time() - ($days * DAY_IN_SECONDS));
        if (self::is_safe_table_name(UCP_TABLE_JOBS)) {
            $table = self::quoted_table(UCP_TABLE_JOBS);
