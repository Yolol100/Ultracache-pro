...
trait UCP_Maintenance_Privacy_Trait {
    ...
    public static function privacy_exporter($email_address, $page = 1) {
        ...
        if (defined('UCP_TABLE_LOGS') && self::is_safe_table_name(UCP_TABLE_LOGS)) {
            $logs_table = self::quoted_table(UCP_TABLE_LOGS);
        ...
        if (defined('UCP_TABLE_DIAGNOSTICS') && self::is_safe_table_name(UCP_TABLE_DIAGNOSTICS)) {
            $diagnostics_table = self::quoted_table(UCP_TABLE_DIAGNOSTICS);
            // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- plugin-owned table identifier is validated and quoted before use; values are prepared.
