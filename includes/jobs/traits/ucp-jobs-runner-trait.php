...
trait UCP_Jobs_Runner_Trait {
    public function run_queue($force = false) {
        ...
        if ($force) {
            ...
            $sql = $wpdb->prepare(
                "SELECT id FROM " . UCP_TABLE_JOBS . " WHERE status IN (%s,%s) AND (locked_until IS NULL OR locked_until < %s) ORDER BY priority ASC, CASE WHEN type IN ('generate_css','remote_css') THEN 0 WHEN type = 'diagnostics_snapshot' THEN 1 WHEN type = 'preload_url' THEN 2 ELSE 1 END ASC, id ASC LIMIT %d",
                'pending',
                'retrying',
            ...
            // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- plugin-owned custom table query with controlled SQL fragments.
            $sql = $wpdb->prepare(
                "SELECT id FROM " . UCP_TABLE_JOBS . " WHERE status IN (%s,%s) AND available_at <= %s AND (locked_until IS NULL OR locked_until < %s) ORDER BY priority ASC, CASE WHEN type IN ('generate_css','remote_css') THEN 0 WHEN type = 'diagnostics_snapshot' THEN 1 WHEN type = 'preload_url' THEN 2 ELSE 1 END ASC, id ASC LIMIT %d",
                'pending',
                'retrying',
                current_time('mysql', true),
                current_time('mysql', true),
    ...
    protected function run_job($type, $payload) {
        switch ($type) {
            case 'generate_css':
                ...
                return $ok;
            case 'remote_css':
                return UCP_Cloud::request_remote_css(UCP_Helpers::enforce_local_url(isset($payload['url']) ? $payload['url'] : home_url('/')));
            case 'cloud_sync':
            ...
            case 'preload_url':
                ...
                if ('' !== $safety_reason || (class_exists('UCP_Preload') && UCP_Preload::is_safety_excluded_url($url))) {
                    UCP_Logger::log('info', 'jobs', 'preload_url_skipped_safety', 'Preload job overgeslagen door centrale safety layer.', array('url' => $url, 'reason' => $safety_reason ? $safety_reason : 'legacy_preload_safety'));
                    return true;
                }
