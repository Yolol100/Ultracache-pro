...
trait UCP_Cache_Storage_Trait {
    public function maybe_serve_cache() {
        ...
        if (file_exists($file)) {
            ...
            if (0 === $ttl || ($modified + $ttl) > time()) {
                ...
                echo UCP_Helpers::read_file($file); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- cached HTML generated by WordPress output buffering.
                exit;
            }
    ...
    public function store_buffer($html) {
        ...
        // AI-PATCH: never persist error/redirect responses as page-cache HTML. A host can return a normal-looking body with HTTP 500.
        if ($status_code >= 300) {
        ...
        $ttl = absint(UCP_Options::get('cache_lifespan', 10)) * HOUR_IN_SECONDS;
        $written = UCP_Helpers::write_file(UCP_Helpers::cache_file_path($current_url), $html);
        if (!$written) {
        ...
        UCP_Diagnostics::record('cache', 'Stored fresh cache file', array('url' => $current_url, 'file' => basename(UCP_Helpers::cache_file_path($current_url))));
        if (!headers_sent()) {
