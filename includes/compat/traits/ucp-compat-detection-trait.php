...
trait UCP_Compat_Detection_Trait {
    ...
    protected static function dynamic_compat_entry_matches($entry) {
        ...
        if (!empty($conditions['plugin_slug'])) {
            if (!is_scalar($conditions['plugin_slug'])) {
                return false;
            }
        ...
        if (!empty($conditions['theme'])) {
            if (!is_scalar($conditions['theme']) || !function_exists('wp_get_theme')) {
                return false;
            }
            $theme = wp_get_theme();
        ...
        if (!empty($conditions['class_exists'])) {
            if (!is_scalar($conditions['class_exists']) || !class_exists((string) $conditions['class_exists'])) {
                return false;
            }
        }
        if (!empty($conditions['function_exists'])) {
            if (!is_scalar($conditions['function_exists']) || !function_exists((string) $conditions['function_exists'])) {
                return false;
            }
        }
        return true;
    ...
    protected static function compatibility_rules_bucket($bucket) {
        ...
        $bucket = UCP_Helpers::sanitize_preg_replace('/[^a-z0-9_-]/i', '', (string) $bucket);
        if ('' === $bucket) {
            return array();
        }
    ...
    public static function compatibility_rules_version() {
        $data = self::compat_json_data('compatibility-rules');
        return !empty($data['schema_version']) && is_scalar($data['schema_version'])
            ? sanitize_text_field((string) $data['schema_version'])
            : '';
    }
    ...
    public static function known_cache_plugins() {
        return array(
            ...
            'seraphinite-accelerator/seraphinite-accelerator.php' => 'Seraphinite Accelerator',
            'tenweb-speed-optimizer/tenweb-speed-optimizer.php' => '10Web Booster',
            'airlift/airlift.php' => 'Airlift',
    ...
    public static function page_cache_plugin_slugs() {
        return array(
            ...
            'seraphinite-accelerator/seraphinite-accelerator.php',
            'tenweb-speed-optimizer/tenweb-speed-optimizer.php',
            'airlift/airlift.php',
        );
    }
    ...
    public static function detected_conflicts() {
        ...
        foreach (self::known_cache_plugins() as $plugin_file => $label) {
            if (in_array($plugin_file, $active_plugins, true)) {
                $conflicts[] = array('type' => 'plugin', 'slug' => $plugin_file, 'label' => $label, 'severity' => 'high');
            }
        }
        if (is_multisite()) {
        ...
        if (file_exists($advanced) && is_readable($advanced)) {
            $content = UCP_Helpers::read_file_head($advanced, 64 * KB_IN_BYTES);
        ...
        if (file_exists(WP_CONTENT_DIR . '/object-cache.php')) {
            $conflicts[] = array('type' => 'dropin', 'slug' => 'object-cache.php', 'label' => __('Persistent object-cache drop-in', 'ultracache-pro'), 'severity' => 'high');
        }
        ...
        if (UCP_Helpers::is_likely_cache_server_present() && !$litespeed_backend_active) {
            $conflicts[] = array('type' => 'server', 'slug' => 'server-cache', 'label' => __('Server-side cache headers detected', 'ultracache-pro'), 'severity' => 'medium');
        }
        if (!empty($_SERVER['HTTP_CF_CACHE_STATUS']) || defined('CLOUDFLARE_PLUGIN_DIR') || class_exists('CF\\WordPress\\Hooks')) {
            $conflicts[] = array('type' => 'edge', 'slug' => 'cloudflare-cache', 'label' => __('Cloudflare cache layer', 'ultracache-pro'), 'severity' => 'medium');
        }
        if (defined('LSCWP_V')) {
            $conflicts[] = array('type' => 'server', 'slug' => 'litespeed-server-cache', 'label' => __('LiteSpeed server cache', 'ultracache-pro'), 'severity' => 'high');
        }
        $unique = array();
        foreach ($conflicts as $conflict) {
            $conflict['features'] = self::feature_conflicts_for_slug(isset($conflict['slug']) ? $conflict['slug'] : '', $conflict['type']);
    ...
    protected static function feature_conflicts_for_slug($slug, $type = 'plugin') {
        ...
        $map = array(
            ...
            'speedycache/speedycache.php' => array('page_cache', 'critical_css', 'delay_js', 'lazyload', 'font_optimization', 'image_optimization'),
            'speedycache-pro/speedycache-pro.php' => array('page_cache', 'critical_css', 'delay_js', 'lazyload', 'font_optimization', 'image_optimization'),
            'seraphinite-accelerator/seraphinite-accelerator.php' => array('page_cache', 'critical_css', 'delay_js', 'lazyload', 'font_optimization', 'cdn_edge_cache'),
            'tenweb-speed-optimizer/tenweb-speed-optimizer.php' => array('page_cache', 'critical_css', 'delay_js', 'lazyload', 'image_optimization', 'cdn_edge_cache'),
            'airlift/airlift.php' => array('page_cache', 'critical_css', 'delay_js', 'lazyload', 'image_optimization', 'cdn_edge_cache'),
